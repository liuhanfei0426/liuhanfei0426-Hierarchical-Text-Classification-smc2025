...
def read_tsv(file_path):
    ...
    labels = df.iloc[:, :label_num].values.tolist()  # label_num = 19，选取的是0-18列
...
with open(output_file, "w") as f:
    with redirect_stdout(f):
        for learning_rate in learning_rates:
            best_model_path = f"best_subclass_1_lr={learning_rate}_batch={train_batch_size}.pt"
            print("*************************************************************************")
            ...
            if metrics['micro']['f1'] > all_best_f1_micro:
                ...
                # 保存预测的结果
                np.savetxt('predict_sub1.tsv', predict.astype(int), delimiter='\t', fmt='%d')
                np.savetxt('gt_sub1.tsv', gt.astype(int), delimiter='\t', fmt='%d')
